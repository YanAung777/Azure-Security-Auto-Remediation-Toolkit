<#
.SYNOPSIS
    Auto-fix: Enable Microsoft Defender for Servers across all subscriptions
#>

param([switch]$WhatIf)

Write-Host "=== Enabling Microsoft Defender for Servers (All Subscriptions) ===" -ForegroundColor Cyan
...
foreach ($subscription in $subscriptions) {
    ...
    try {
        ...
        if (-not $WhatIf) {
            try {
                Set-AzSecurityPricing -Name "VirtualMachines" -PricingTier "Standard" -ErrorAction Stop
                Write-Host "  ✓ Microsoft Defender for Servers enabled" -ForegroundColor Green
            } catch {
                Write-Host "  ✗ Failed to enable Defender: $($_.Exception.Message)" -ForegroundColor Red
            }
        } else {
            Write-Host "  [WhatIf] Would enable Defender for Servers" -ForegroundColor Gray
        }
    } catch {
